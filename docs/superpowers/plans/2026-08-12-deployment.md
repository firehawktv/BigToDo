...
- **The VPS already runs other services behind an existing nginx instance** (confirmed by the user, not assumed) — this plan adds one new CloudPanel-managed nginx site (CloudPanel generates the server block; there is no hand-authored `sites-available` file and no certbot certificate — see the next bullet) for `todo.cooney.fun`; it must not touch, restart in a way that drops connections for, or reconfigure any existing site's config. `nginx -t` (config syntax check) before every reload is mandatory, not optional, because a syntax error in this app's block would take down every other site nginx serves.
...
**Confirmed by this task's own live check: certbot is not installed on this box at all.** TLS is obtained instead through CloudPanel's own Let's Encrypt integration, `clpctl lets-encrypt:install:certificate` — this is what Tasks 4 and 5 use throughout, not `certbot --nginx` or any other certbot invocation (there is no certbot fallback path in this plan; if a future box genuinely does have certbot instead of CloudPanel, that's a different plan, not a runtime branch of this one).
...
```bash
sudo clpctl lets-encrypt:install:certificate --domainName=todo.cooney.fun
```

(Confirm this exact flag name against current `clpctl` usage output at execution time — run bare `clpctl` (no `--help`; Task 4's investigation found `--help` is not a valid flag on this CloudPanel version, it errors with `"help" does not exist`) to see full subcommand usage. CloudPanel's own nightly self-update means the exact flag syntax should be re-confirmed live, not assumed frozen since Task 4 ran.)
...
- [ ] `docker-compose.prod.yml` binds the backend to `127.0.0.1` only, and Postgres publishes no host port at all — no public port exposure for either service. (Not "both services bound to `127.0.0.1`": Postgres deliberately has zero host-port mapping, stricter than a `127.0.0.1`-bound one — do not "fix" this by adding a `127.0.0.1:5432:5432` mapping.)
